...
<xsl:stylesheet version="2.0"
  ...
  <xsl:param name="inDir" select="'sms-sjd2X_src'"/>
  <xsl:param name="paraDir" select="'overlap_id_set'"/>
  <xsl:param name="cIndex" select="'01'"/>
  <xsl:param name="outDir" select="concat('_complement_stuff_', $cIndex)"/>
  ...
  <xsl:template match="/" name="main">
	...
	<xsl:call-template name="processFile">
	  ...
	  <xsl:with-param name="parallelDir" select="document(concat($paraDir, '/', 'sms-ids.xml'))"/>
	</xsl:call-template>
      </xsl:for-each>
  ...
  <xsl:template name="processFile">
	...
	<xsl:for-each select="$file//e">
	  ...
	  <xsl:if test="not($current_id = $parallelDir//id)">
